...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet)  
    {
        ...
        //finding the row where the longittute and latitude are 
        int row = 0;
        ...
        //storing the possibilities value
        double dryValue = drywet[row][month];
        double wetValue = wetwet[row][month];
        ...
        {
            randValue = StdRandom.uniform();
            if (weather[i-1] == WET)
            {
                if(randValue < wetValue)
                    weather[i] = WET;
                else
                    weather[i] = DRY;
            }
    ...
    {
        int current = 0; int biggest = 0; int last =0;
        for (int i =0; i< forecast.length; i++)
        {
    ...
    public static void main (String[] args) {
        ...
        //StdRandom.setSeed(1604629740169L);
        StdRandom.setSeed(1604680648335L);
